...
def main():
    ...
    results.append(test_endpoint("ユーザー登録", "POST", "/api/v1/auth/register", json=register_data))
    ...
    results.append(test_endpoint("レース予測（認証なし）", "POST", "/api/v1/predictions/race", json=prediction_data))
    
    # バッチ予測
    batch_data = {
        "races": [prediction_data]
    }
    results.append(test_endpoint("バッチ予測（認証なし）", "POST", "/api/v1/predictions/batch", json=batch_data))
