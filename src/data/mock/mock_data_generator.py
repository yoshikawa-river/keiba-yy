...
class MockDataGenerator:
    ...
    def _generate_horses_master(self):
        ...
        for i in range(self.num_horses):
            ...
            birth_date = self.start_date - timedelta(days=random.randint(1095, 2190))  # 3-6歳
    ...
    def _generate_jockeys_master(self):
        ...
        selected_names = random.sample(JOCKEY_NAMES, min(self.num_jockeys, len(JOCKEY_NAMES)))
        ...
        for i, name in enumerate(selected_names):
            ...
            jockey_data = {
                ...
                'SEINENGAPPI': (date(1970, 1, 1) + timedelta(days=random.randint(0, 18250))).strftime("%Y%m%d"),
    ...
    def _generate_trainers_master(self):
        ...
        selected_names = random.sample(TRAINER_NAMES, min(self.num_trainers, len(TRAINER_NAMES)))
        ...
        for i, name in enumerate(selected_names):
            ...
            trainer_data = {
                ...
                'SEINENGAPPI': (date(1950, 1, 1) + timedelta(days=random.randint(0, 25550))).strftime("%Y%m%d"),
        ...
        while current_date <= self.end_date:
            ...
            if self._should_have_racing(current_date):
                ...
                for race_num in range(1, race_count + 1):
                    race_data = self._generate_single_race(current_date, keibajo_code, race_num)
    ...
    def _generate_single_race(self, race_date: date, keibajo_code: str, race_num: int) -> Dict:
        """単一レースのデータを生成"""
    ...
    def _calculate_realistic_time(self, distance: int) -> float:
        ...
        base_times = {
            ...
            1800: 108.0, 2000: 120.0, 2200: 132.0, 2400: 144.0,
            2500: 150.0, 3000: 186.0, 3200: 200.0, 3400: 214.0,
        }
    ...
    def _convert_race_data(self, raw_data: Dict[str, pd.DataFrame]) -> pd.DataFrame:
        ...
        # 結合して統合データを作成
        race_data = entries.merge(races, on='RACE_CODE', how='left', suffixes=('_entry', '_race'))
        
        # カラム名を特徴量抽出器が期待する形式に変換
        converted = race_data.rename(columns={
            'RACE_CODE': 'race_id',
            'KETTO_TOROKU_BANGO': 'horse_id',
            'KISHU_CODE': 'jockey_id',
            'CHOKYOSHI_CODE': 'trainer_id',
        ...
        converted['finish_position'] = pd.to_numeric(converted['finish_position'], errors='coerce')
        converted['weight_carried'] = pd.to_numeric(converted['weight_carried'], errors='coerce')
        
        # 日付変換 - どちらのカラムセットを使うか確認
        kaisai_nen_col = 'KAISAI_NEN_entry' if 'KAISAI_NEN_entry' in race_data.columns else 'KAISAI_NEN'
        kaisai_gappi_col = 'KAISAI_GAPPI_entry' if 'KAISAI_GAPPI_entry' in race_data.columns else 'KAISAI_GAPPI'
        
        converted['race_date'] = pd.to_datetime(
            race_data[kaisai_nen_col] + race_data[kaisai_gappi_col], 
            format='%Y%m%d'
        )
        ...
        result_columns = [
            ...
            'distance', 'venue_code', 'race_class', 'finish_position',
            'finish_time', 'weight_carried', 'race_date'
        ]
