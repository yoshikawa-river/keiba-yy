...
def test_feature_extraction_compatibility():
    ...
    # 必須カラムの存在確認
    required_columns = [
        'race_id', 'horse_id', 'distance', 'finish_position', 
        'race_date', 'venue', 'race_class'
    ]
    ...
    assert pd.api.types.is_numeric_dtype(races_df['finish_position']), "finish_position が数値型ではありません"
    assert pd.api.types.is_datetime64_any_dtype(races_df['race_date']), "race_date が日付型ではありません"
...
def test_csv_output():
    ...
    # ファイル存在確認
    expected_files = [
        'horses_master.csv', 'jockeys_master.csv', 'trainers_master.csv',
        'owners_master.csv', 'races.csv', 'race_entries.csv'
    ]
