...
class TimeSeriesProcessor:
        ...
        try:
            for col in date_columns:
                ...
                if df_processed[col].dt.tz is None:
                    df_processed[col] = df_processed[col].dt.tz_localize(target_timezone)
                else:
            ...
            raise DataProcessingError(f"日付データ標準化中にエラーが発生しました: {str(e)}")
    ...
    def create_time_features(
        ...
        logger.info(f"時系列特徴量生成開始: base_column={date_column}, features={len(features)}")
        ...
        try:
            ...
            if 'is_weekend' in features:
                df_processed[f'{date_column}_is_weekend'] = (dt.dayofweek >= 5).astype(int)
            if 'is_month_start' in features:
                df_processed[f'{date_column}_is_month_start'] = dt.is_month_start.astype(int)
            if 'is_month_end' in features:
                df_processed[f'{date_column}_is_month_end'] = dt.is_month_end.astype(int)
            if 'is_quarter_start' in features:
                df_processed[f'{date_column}_is_quarter_start'] = dt.is_quarter_start.astype(int)
            if 'is_quarter_end' in features:
                df_processed[f'{date_column}_is_quarter_end'] = dt.is_quarter_end.astype(int)
            ...
            if 'month_sin' in features:
                df_processed[f'{date_column}_month_sin'] = np.sin(2 * np.pi * dt.month / 12)
            if 'month_cos' in features:
                df_processed[f'{date_column}_month_cos'] = np.cos(2 * np.pi * dt.month / 12)
            ...
            if 'dayofweek_sin' in features:
                df_processed[f'{date_column}_dayofweek_sin'] = np.sin(2 * np.pi * dt.dayofweek / 7)
            if 'dayofweek_cos' in features:
                df_processed[f'{date_column}_dayofweek_cos'] = np.cos(2 * np.pi * dt.dayofweek / 7)
            
            self.time_features.extend([f'{date_column}_{feat}' for feat in features if f'{date_column}_{feat}' in df_processed.columns])
            ...
            raise DataProcessingError(f"時系列特徴量生成中にエラーが発生しました: {str(e)}")
    ...
    def create_lag_features(
        ...
        logger.info(f"ラグ特徴量作成開始: target_columns={len(target_columns)}, lag_periods={lag_periods}")
        ...
        try:
            for col in target_columns:
                ...
                for lag in lag_periods:
                    ...
                    if group_columns:
                        # グループごとにラグ特徴量を作成
                        df_processed[lag_col_name] = df_processed.groupby(group_columns)[col].shift(lag)
                    else:
                        # 全体でラグ特徴量を作成
            ...
            for col in target_columns:
                ...
                for window in windows:
                    ...
                    if group_columns:
                        df_processed[ma_col_name] = df_processed.groupby(group_columns)[col].transform(
                            lambda x: x.rolling(window=window, min_periods=1).mean()
                        )
                    else:
                        df_processed[ma_col_name] = df_processed[col].rolling(window=window, min_periods=1).mean()
                    
                    logger.debug(f"作成: {ma_col_name}")
            
            logger.info(f"ラグ特徴量作成完了")
            
            return df_processed
            
        except Exception as e:
            raise DataProcessingError(f"ラグ特徴量作成中にエラーが発生しました: {str(e)}")
    ...
    def create_time_diff_features(
        ...
        try:
            ...
            # 前のレコードとの時間差
            if group_column:
                df_processed[f'{date_column}_diff_days'] = df_processed.groupby(group_column)[date_column].diff().dt.days
                df_processed[f'{date_column}_diff_hours'] = df_processed.groupby(group_column)[date_column].diff().dt.total_seconds() / 3600
            else:
                df_processed[f'{date_column}_diff_days'] = df_processed[date_column].diff().dt.days
                df_processed[f'{date_column}_diff_hours'] = df_processed[date_column].diff().dt.total_seconds() / 3600
            ...
            if reference_dates:
                for ref_name, ref_date in reference_dates.items():
                    ...
                    df_processed[f'{date_column}_from_{ref_name}_days'] = (df_processed[date_column] - ref_date).dt.days
            ...
            if group_column:
                # 平均間隔
                mean_diff = df_processed.groupby(group_column)[f'{date_column}_diff_days'].transform('mean')
                df_processed[f'{date_column}_diff_from_mean'] = df_processed[f'{date_column}_diff_days'] - mean_diff
                
                # 最小・最大間隔
                df_processed[f'{date_column}_min_diff'] = df_processed.groupby(group_column)[f'{date_column}_diff_days'].transform('min')
                df_processed[f'{date_column}_max_diff'] = df_processed.groupby(group_column)[f'{date_column}_diff_days'].transform('max')
            ...
            raise DataProcessingError(f"時間差特徴量作成中にエラーが発生しました: {str(e)}")
    ...
    def create_seasonal_features(
        ...
        try:
            ...
            raise DataProcessingError(f"季節性特徴量作成中にエラーが発生しました: {str(e)}")