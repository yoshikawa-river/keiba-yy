...
class BatchProcessor:
    ...
    def process_all(
        ...
        try:
            ...
            for file_type in processing_order:
                ...
                logger.info(f"{file_type.value} ファイルの処理開始: {len(type_files)}件")
                
                for i, csv_file in enumerate(type_files):
                    # 進捗コールバック
                    if progress_callback:
                        progress_callback(result.processed_files + 1, result.total_files)
    ...
    def _group_files_by_type(self, csv_files: List[CSVFile]) -> Dict[FileType, List[CSVFile]]:
        """ファイルをタイプ別にグループ化"""
        grouped = {}
        ...
        with db_manager.get_session() as session:
            ...
            try:
                ...
                if self.dry_run:
                    # ドライランの場合はロールバック
                    session.rollback()
                    logger.info(f"ドライラン: {csv_file.path.name} の変更をロールバック")
                else:
                    # 実際の処理の場合はコミット
                    session.commit()
    ...
    def validate_data_quality(self, csv_files: List[CSVFile]) -> Dict[str, Any]:
        ...
        for csv_file in csv_files:
            ...
            if csv_file.file_size_mb > 100:
                file_issues.append(f"ファイルサイズが大きすぎます: {csv_file.file_size_mb:.1f}MB")
            elif csv_file.file_size_mb < 0.001:
                file_issues.append(f"ファイルが空の可能性があります: {csv_file.file_size_mb:.3f}MB")
            else:
                file_score += 25
            ...
            quality_report["issues"].extend([
                {"file": str(csv_file.path), "issue": issue}
