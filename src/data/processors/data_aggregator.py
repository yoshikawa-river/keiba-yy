...
from src.data.models.horse import Horse
from src.data.models.race import Race, RaceHorse
from src.data.models.result import RaceResult
...
class DataAggregator:
    ...
    def aggregate_horse_performance(
        ...
        try:
            ...
            if 'finish_position' in result_columns:
                col = result_columns['finish_position']
                horse_stats[f'{col}_win_rate'] = horse_stats[f'{col}_win'] / horse_stats[f'{col}_count']
                horse_stats[f'{col}_place_rate'] = horse_stats[f'{col}_place'] / horse_stats[f'{col}_count']
                horse_stats[f'{col}_show_rate'] = horse_stats[f'{col}_show'] / horse_stats[f'{col}_count']
                
                # ROI（回収率）の計算（簡易版：賞金/出走回数）
                if 'prize_money' in result_columns:
                    prize_col = result_columns['prize_money']
                    horse_stats['roi'] = horse_stats[f'{prize_col}_sum'] / (horse_stats[f'{col}_count'] * 100)  # 100円賭けと仮定
            
            # キャッシュに保存
            cache_key = f"horse_{horse_id_column}_{str(group_by)}"
            ...
            raise DataProcessingError(f"馬別成績集計中にエラーが発生しました: {str(e)}")
    ...
    def aggregate_jockey_performance(
        ...
        try:
            ...
            if 'finish_position' in result_columns:
                col = result_columns['finish_position']
                jockey_stats[f'{col}_win_rate'] = jockey_stats[f'{col}_win'] / jockey_stats[f'{col}_count']
                jockey_stats[f'{col}_place_rate'] = jockey_stats[f'{col}_place'] / jockey_stats[f'{col}_count']
                jockey_stats[f'{col}_show_rate'] = jockey_stats[f'{col}_show'] / jockey_stats[f'{col}_count']
            ...
            raise DataProcessingError(f"騎手別成績集計中にエラーが発生しました: {str(e)}")
    ...
    def aggregate_trainer_performance(
        ...
        try:
            ...
            if 'finish_position' in result_columns:
                col = result_columns['finish_position']
                trainer_stats[f'{col}_win_rate'] = trainer_stats[f'{col}_win'] / trainer_stats[f'{col}_count']
                trainer_stats[f'{col}_place_rate'] = trainer_stats[f'{col}_place'] / trainer_stats[f'{col}_count']
                trainer_stats[f'{col}_show_rate'] = trainer_stats[f'{col}_show'] / trainer_stats[f'{col}_count']
            ...
            raise DataProcessingError(f"調教師別成績集計中にエラーが発生しました: {str(e)}")
    ...
    def aggregate_course_performance(
        ...
        try:
            ...
            raise DataProcessingError(f"コース別成績集計中にエラーが発生しました: {str(e)}")
    ...
    def aggregate_recent_performance(
        ...
        try:
            # 日付でソート
            df_sorted = df.sort_values([entity_id_column, date_column], ascending=[True, False])
            ...
            for key, col in result_columns.items():
                if key == 'finish_position':
                    ...
                    aggregations[f'{col}_recent_avg'] = (col, 'mean')
                    aggregations[f'{col}_recent_best'] = (col, 'min')
                    aggregations[f'{col}_trend'] = (col, lambda x: x.diff().mean())  # タイムの改善傾向
                elif key == 'prize_money':
                    aggregations[f'{col}_recent_sum'] = (col, 'sum')
                    aggregations[f'{col}_recent_avg'] = (col, 'mean')
            
            # エンティティごとに集計
            recent_stats = df_filtered.groupby(entity_id_column).agg(**aggregations).reset_index()
            ...
            raise DataProcessingError(f"直近成績集計中にエラーが発生しました: {str(e)}")
    ...
    def create_performance_trends(
        ...
        try:
            ...
            # 移動平均
            df_sorted[f'{metric_column}_ma_{window_size}'] = df_sorted.groupby(entity_id_column)[metric_column].transform(
                lambda x: x.rolling(window=window_size, min_periods=1).mean()
            )
            
            # 移動標準偏差
            df_sorted[f'{metric_column}_std_{window_size}'] = df_sorted.groupby(entity_id_column)[metric_column].transform(
            ...
            df_sorted[f'{metric_column}_trend_{window_size}'] = df_sorted.groupby(entity_id_column)[metric_column].transform(
                lambda x: x.rolling(window=window_size, min_periods=2).apply(calc_trend)
            )
            
            # 前回との差分
            df_sorted[f'{metric_column}_diff'] = df_sorted.groupby(entity_id_column)[metric_column].diff()
            
            # 累積統計
            df_sorted[f'{metric_column}_cumsum'] = df_sorted.groupby(entity_id_column)[metric_column].cumsum()
            df_sorted[f'{metric_column}_cumcount'] = df_sorted.groupby(entity_id_column).cumcount() + 1
            df_sorted[f'{metric_column}_cumavg'] = (
                df_sorted[f'{metric_column}_cumsum'] / df_sorted[f'{metric_column}_cumcount']
            )
            ...
            raise DataProcessingError(f"パフォーマンストレンド作成中にエラーが発生しました: {str(e)}")