...
class DataCleaner:
    ...
    def handle_missing_values(
        ...
        try:
            if strategy == 'drop':
                ...
                numeric_columns = df_processed[columns].select_dtypes(include=[np.number]).columns
                categorical_columns = df_processed[columns].select_dtypes(include=['object']).columns
            ...
            raise DataProcessingError(f"欠損値処理中にエラーが発生しました: {str(e)}")
    ...
    def detect_outliers(
        ...
        try:
            ...
            raise DataProcessingError(f"外れ値検出中にエラーが発生しました: {str(e)}")
    ...
    def handle_outliers(
        ...
        try:
            ...
            raise DataProcessingError(f"外れ値処理中にエラーが発生しました: {str(e)}")
    ...
    def normalize_data(
        ...
        try:
            ...
            raise DataProcessingError(f"データ正規化中にエラーが発生しました: {str(e)}")
    ...
    def encode_categorical(
        ...
        logger.info(f"カテゴリ変数エンコーディング開始: method={method}, columns={len(columns)}")
        ...
        try:
            if method == 'label':
                ...
                logger.warning("ターゲットエンコーディングは未実装です。ラベルエンコーディングを使用します。")
                return self.encode_categorical(df, columns, method='label')
                
            else:
                raise ValueError(f"不明なエンコーディング方法: {method}")
            
            logger.info(f"カテゴリ変数エンコーディング完了: 処理後のカラム数={len(df_processed.columns)}")
            
            return df_processed
            
        except Exception as e:
            raise DataProcessingError(f"カテゴリ変数エンコーディング中にエラーが発生しました: {str(e)}")
    ...
    def clean_data(
        ...
        try:
            ...
            if numeric_columns:
                outlier_flags, _ = self.detect_outliers(df_cleaned, columns=numeric_columns, method=outlier_method)
                df_cleaned = self.handle_outliers(df_cleaned, outlier_flags, method=outlier_handling)
            
            # 3. カテゴリ変数エンコーディング
            categorical_columns = df_cleaned.select_dtypes(include=['object']).columns.tolist()
            if categorical_columns:
                df_cleaned = self.encode_categorical(df_cleaned, columns=categorical_columns, method=encode_method)
            
            # 4. データ正規化
            numeric_columns = df_cleaned.select_dtypes(include=[np.number]).columns.tolist()
            if numeric_columns:
                df_cleaned = self.normalize_data(df_cleaned, columns=numeric_columns, method=normalize_method)
            ...
            raise DataProcessingError(f"データクレンジング中にエラーが発生しました: {str(e)}")