...
class DataQualityReporter:
        ...
        try:
            ...
            summary = {
                ...
                "columns_without_missing": int(
                    (missing_df["missing_count"] == 0).sum()
                ),
                ...
                "columns_over_50pct_missing": int(
                    (missing_df["missing_ratio"] > 0.5).sum()
                ),
                "columns_over_80pct_missing": int(
                    (missing_df["missing_ratio"] > 0.8).sum()
                ),
            }
            ...
            report = {
                ...
                "recommendations": self._generate_missing_value_recommendations(
                    missing_df
                ),
            }
            ...
            # 可視化
            self._visualize_missing_values(
                missing_df, output_file.replace(".json", ".png")
            )
    ...
    def _generate_missing_value_recommendations(
        ...
        medium_missing = missing_df[
            (missing_df["missing_ratio"] > 0.2) & (missing_df["missing_ratio"] <= 0.8)
        ]
        ...
        categorical_missing = missing_df[
            (missing_df["dtype"] == "object") & (missing_df["missing_count"] > 0)
        ]
    ...
    def _visualize_missing_values(self, missing_df: pd.DataFrame, output_file: str):
        ...
        ax3 = axes[1, 0]
        ax3.hist(
            missing_df[missing_df["missing_ratio"] > 0]["missing_ratio"] * 100,
            bins=20,
            edgecolor="black",
        )
        ax3.set_xlabel("欠損率 (%)")
    ...
    def generate_distribution_report(
        ...
        try:
            ...
            self._visualize_distributions(
                df, columns[:12], output_file.replace(".json", ".png")
            )
    ...
    def _generate_distribution_summary(self, stats: list[dict]) -> dict[str, Any]:
        ...
        return {
            ...
            "highly_skewed_columns": stats_df[stats_df["skewness"].abs() > 2][
                "column"
            ].tolist(),
            "high_kurtosis_columns": stats_df[stats_df["kurtosis"].abs() > 7][
                "column"
            ].tolist(),
        }
    ...
    def generate_outlier_report(
        ...
        try:
            ...
            if output_file is None:
                output_file = (
                    f"outlier_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
                )
            ...
            self._visualize_outliers(
                df, outlier_stats[:9], output_file.replace(".json", ".png")
            )
    ...
    def _generate_outlier_summary(self, stats: list[dict]) -> dict[str, Any]:
        ...
        return {
            ...
            "columns_over_5pct_outliers": stats_df[stats_df["outlier_ratio"] > 0.05][
                "column"
            ].tolist(),
            "columns_over_10pct_outliers": stats_df[stats_df["outlier_ratio"] > 0.10][
                "column"
            ].tolist(),
        }
    ...
    def _generate_outlier_recommendations(self, stats: list[dict]) -> list[str]:
        ...
        # 外れ値が少ないカラム
        low_outlier = stats_df[
            (stats_df["outlier_ratio"] > 0) & (stats_df["outlier_ratio"] <= 0.05)
        ]
