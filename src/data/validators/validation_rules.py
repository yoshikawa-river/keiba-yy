...
class ValidationRules:
    ...
    def get_race_schema() -> Schema:
        """レース情報用スキーマを取得"""
        return Schema("race", [
            SchemaField("race_key", str, required=True, custom_validator=ValidationRules.jra_race_key_validator),
            SchemaField("race_date", str, required=True, custom_validator=ValidationRules.japanese_date_validator),
            SchemaField("race_number", int, required=True, min_value=1, max_value=12),
            SchemaField("race_name", str, required=True),
            SchemaField("venue_name", str, required=True),
            SchemaField("race_type", str, required=True, enum_values=["芝", "ダート", "障害"]),
            SchemaField("distance", int, required=True, min_value=800, max_value=3600),
            SchemaField("weather", str, enum_values=["晴", "曇", "雨", "小雨", "雪", None]),
            SchemaField("track_condition", str, enum_values=["良", "稍重", "重", "不良", None]),
            SchemaField("grade", str, enum_values=["G1", "G2", "G3", "OP", "L", None]),
    ...
    def get_horse_schema() -> Schema:
        ...
        return Schema("horse", [
            ...
            SchemaField("birth_date", str, custom_validator=ValidationRules.japanese_date_validator),
            SchemaField("color", str, enum_values=["鹿毛", "黒鹿毛", "栗毛", "栃栗毛", "芦毛", "白毛", None]),
    ...
    def get_race_result_schema() -> Schema:
        """レース結果用スキーマを取得"""
        return Schema("race_result", [
            SchemaField("race_key", str, required=True, custom_validator=ValidationRules.jra_race_key_validator),
            SchemaField("post_position", int, required=True, min_value=1, max_value=28),
            SchemaField("horse_key", str, required=True, pattern=r"^\d{8,}$"),
            SchemaField("jockey_key", str, required=True, pattern=r"^\d{4,}$"),
            SchemaField("weight_carried", float, required=True, min_value=48.0, max_value=65.0),
    ...
    def get_odds_schema() -> Schema:
        ...
        return Schema("odds", [
            ...
            SchemaField("odds_type", str, required=True, enum_values=["win", "place", "exacta", "quinella", "wide", "trio", "trifecta"]),
...
class DataQualityRules:
    ...
    @staticmethod
    def check_completeness(data: Dict[str, Any], required_fields: List[str]) -> Dict[str, float]:
        """
        データの完全性をチェック
        ...
        return {
            ...
            "required_completeness": required_filled / len(required_fields) if required_fields else 1.0,
