...
class PedigreeFeatureExtractor:
    ...
    def extract_sire_features(
        ...
        try:
            ...
            for idx, row in df.iterrows():
                ...
                if sire_id and not pd.isna(sire_id):
                    ...
                    if len(sire_data) > 0:
                        ...
                        stats['sire_win_count'] = (sire_data['finish_position'] == 1).sum()
                        stats['sire_win_rate'] = stats['sire_win_count'] / stats['sire_progeny_count']
                        stats['sire_place_rate'] = (sire_data['finish_position'] <= 2).sum() / stats['sire_progeny_count']
                        stats['sire_show_rate'] = (sire_data['finish_position'] <= 3).sum() / stats['sire_progeny_count']
                        stats['sire_avg_finish'] = sire_data['finish_position'].mean()
                        stats['sire_earnings_avg'] = sire_data['prize_money'].mean() if 'prize_money' in sire_data else 0
                        ...
                        if 'distance' in row and 'distance' in sire_data.columns:
                            ...
                            same_category = sire_data[
                                sire_data['distance'].apply(self._get_distance_category) == distance_category
                            ]
                            
                            if len(same_category) > 0:
                                stats[f'sire_{distance_category}_win_rate'] = (
                                    (same_category['finish_position'] == 1).sum() / len(same_category)
                                )
                                stats[f'sire_{distance_category}_count'] = len(same_category)
                        
                        # コース別成績（芝・ダート）
                        if 'track_type' in row and 'track_type' in sire_data.columns:
                            current_track = row['track_type']
                            same_track = sire_data[sire_data['track_type'] == current_track]
                            
                            if len(same_track) > 0:
                                stats[f'sire_{current_track}_win_rate'] = (
                                    (same_track['finish_position'] == 1).sum() / len(same_track)
                                )
                                stats[f'sire_{current_track}_count'] = len(same_track)
                        
                        # 重要種牡馬フラグ
                        if stats['sire_win_rate'] > 0.15 and stats['sire_progeny_count'] > 50:
            ...
            logger.info(f"父馬の成績特徴量抽出完了: 特徴量数={len(sire_df.columns) if sire_stats else 0}")
            
            return df_features
            
        except Exception as e:
            raise FeatureExtractionError(f"父馬の成績特徴量抽出中にエラーが発生しました: {str(e)}")
    ...
    def extract_dam_sire_features(
        ...
        try:
                ...
                if dam_sire_id and not pd.isna(dam_sire_id):
                    ...
                    if len(dam_sire_data) > 0:
                        ...
                        stats['dam_sire_win_rate'] = (
                            (dam_sire_data['finish_position'] == 1).sum() / stats['dam_sire_progeny_count']
                        )
                        stats['dam_sire_avg_finish'] = dam_sire_data['finish_position'].mean()
                        
                        # スタミナ指標（長距離での成績）
                        if 'distance' in dam_sire_data.columns:
                            long_distance = dam_sire_data[dam_sire_data['distance'] >= 2200]
                            if len(long_distance) > 0:
                                stats['dam_sire_stamina_index'] = (
                                    (long_distance['finish_position'] <= 3).sum() / len(long_distance)
                                )
                            else:
                        ...
                        # スピード指標（短距離での成績）
                        if 'distance' in dam_sire_data.columns:
                            short_distance = dam_sire_data[dam_sire_data['distance'] <= 1400]
                            if len(short_distance) > 0:
                                stats['dam_sire_speed_index'] = (
                                    (short_distance['finish_position'] <= 3).sum() / len(short_distance)
                                )
                            else:
            ...
            logger.info(f"母父馬の成績特徴量抽出完了")
            
            return df_features
            
        except Exception as e:
            raise FeatureExtractionError(f"母父馬の成績特徴量抽出中にエラーが発生しました: {str(e)}")
    ...
    def extract_sibling_features(
        ...
        try:
            ...
            if 'dam_id' not in df.columns:
                df_features = df_features.merge(
                    horse_pedigree[['horse_id', 'dam_id']],
                    on='horse_id',
                    how='left'
                )
                ...
                if dam_id and not pd.isna(dam_id):
                    ...
                    if len(siblings) > 0:
                        ...
                        if len(sibling_data) > 0:
                            ...
                            stats['sibling_win_count'] = (sibling_data['finish_position'] == 1).sum()
                            stats['sibling_win_rate'] = stats['sibling_win_count'] / stats['sibling_race_count']
                            stats['sibling_avg_finish'] = sibling_data['finish_position'].mean()
                            stats['sibling_earnings_total'] = sibling_data['prize_money'].sum() if 'prize_money' in sibling_data else 0
                            ...
                            if 'race_class' in sibling_data.columns:
                                ...
                                stats['has_g1_winner_sibling'] = 1 if len(g1_wins) > 0 else 0
                            else:
            ...
            logger.info(f"兄弟馬の成績特徴量抽出完了")
            
            return df_features
            
        except Exception as e:
            raise FeatureExtractionError(f"兄弟馬の成績特徴量抽出中にエラーが発生しました: {str(e)}")
    
    def extract_bloodline_affinity(
        self,
        df: pd.DataFrame,
        bloodline_cross_performance: pd.DataFrame
    ) -> pd.DataFrame:
        """血統相性特徴量の抽出
        ...
        try:
            ...
            for idx, row in df.iterrows():
                ...
                if sire_id and dam_sire_id and not pd.isna(sire_id) and not pd.isna(dam_sire_id):
                    # 同じ配合パターンの成績
                    ...
                    if len(same_cross) > 0:
                        ...
                        stats['cross_win_rate'] = (same_cross['finish_position'] == 1).sum() / len(same_cross)
                        stats['cross_avg_finish'] = same_cross['finish_position'].mean()
                        stats['is_golden_cross'] = 1 if stats['cross_win_rate'] > 0.2 and stats['cross_count'] > 10 else 0
                    else:
                        stats['cross_count'] = 0
                    ...
                    # 血統系統の特徴
                    stats['is_inbreed'] = self._check_inbreeding(sire_id, dam_sire_id, row)
                else:
                    stats = {
                        'cross_count': 0,
                        'cross_win_rate': 0,
            ...
            raise FeatureExtractionError(f"血統相性特徴量抽出中にエラーが発生しました: {str(e)}")
    ...
    def extract_all_pedigree_features(
        ...
        try:
            ...
            raise FeatureExtractionError(f"全血統特徴量抽出中にエラーが発生しました: {str(e)}")