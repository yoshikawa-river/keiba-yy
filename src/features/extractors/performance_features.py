...
class PerformanceFeatureExtractor:
    ...
    def extract_career_performance(
        ...
        try:
            ...
            for idx, row in df.iterrows():
                ...
                if len(past_results) == 0:
                    ...
                    stats = {
                        ...
                        'career_earnings': past_results['prize_money'].sum() if 'prize_money' in past_results else 0,
            ...
            raise FeatureExtractionError(f"通算成績特徴量抽出中にエラーが発生しました: {str(e)}")
    ...
    def extract_recent_performance(
        ...
        try:
            ...
            for n in n_recent:
                ...
                for idx, row in df.iterrows():
                    ...
                    past_results = performance_history[
                        (performance_history[entity_column] == entity_id) &
                        (performance_history[date_column] < current_date)
                    ].sort_values(date_column, ascending=False).head(n)
                    ...
                    if len(past_results) == 0:
                        ...
                        # トレンド計算（改善=正、悪化=負）
                        if len(positions) > 1:
                            trend = np.polyfit(range(len(positions)), positions, 1)[0] * -1
                        else:
                            trend = 0
                        ...
                        stats = {
                            ...
                            f'recent{n}_consistency': positions.std() if len(positions) > 1 else np.nan
                        }
            ...
            logger.info(f"直近成績特徴量抽出完了")
            
            return df_features
            
        except Exception as e:
            raise FeatureExtractionError(f"直近成績特徴量抽出中にエラーが発生しました: {str(e)}")
    ...
    def extract_course_performance(
        ...
        try:
            ...
            for idx, row in df.iterrows():
                ...
                if current_course:
                    ...
                    if len(same_course) > 0:
                        ...
                        stats['same_course_wins'] = (same_course['finish_position'] == 1).sum()
                        stats['same_course_win_rate'] = stats['same_course_wins'] / stats['same_course_starts']
                        stats['same_course_avg_finish'] = same_course['finish_position'].mean()
                    else:
                ...
                if 'turn_direction' in row:
                    ...
                    if len(same_turn) > 0:
                        ...
                        stats[f'{turn_dir}_turn_win_rate'] = (same_turn['finish_position'] == 1).sum() / len(same_turn)
                        stats[f'{turn_dir}_turn_avg_finish'] = same_turn['finish_position'].mean()
            ...
            raise FeatureExtractionError(f"コース別成績特徴量抽出中にエラーが発生しました: {str(e)}")
    ...
    def extract_distance_performance(
        ...
        try:
            ...
            for idx, row in df.iterrows():
                ...
                if current_distance:
                    ...
                    for i, (min_dist, max_dist) in enumerate(distance_ranges):
                        if min_dist <= current_distance <= max_dist:
                            ...
                            if len(same_category) > 0:
                                ...
                                stats[f'{category_name}_wins'] = (same_category['finish_position'] == 1).sum()
                                stats[f'{category_name}_win_rate'] = stats[f'{category_name}_wins'] / stats[f'{category_name}_starts']
                                stats[f'{category_name}_avg_finish'] = same_category['finish_position'].mean()
                            else:
                    ...
                    if len(exact_distance) > 0:
                        ...
                        stats['exact_distance_win_rate'] = (exact_distance['finish_position'] == 1).sum() / len(exact_distance)
            ...
            raise FeatureExtractionError(f"距離別成績特徴量抽出中にエラーが発生しました: {str(e)}")
    ...
    def extract_track_condition_performance(
        ...
        try:
            ...
            for idx, row in df.iterrows():
                ...
                if current_track_type:
                    ...
                    if len(same_track_type) > 0:
                        ...
                        stats[f'{current_track_type}_win_rate'] = (same_track_type['finish_position'] == 1).sum() / len(same_track_type)
                        stats[f'{current_track_type}_avg_finish'] = same_track_type['finish_position'].mean()
                ...
                if current_condition:
                    ...
                    good_condition = performance_history[
                        (performance_history[entity_column] == entity_id) &
                        (performance_history[date_column] < current_date) &
                        (performance_history['track_condition'].isin(['firm', 'good']))
                    ]
                    
                    if len(good_condition) > 0:
                        stats['good_condition_starts'] = len(good_condition)
                        stats['good_condition_win_rate'] = (good_condition['finish_position'] == 1).sum() / len(good_condition)
                    
                    # 重・不良馬場
                    bad_condition = performance_history[
                        (performance_history[entity_column] == entity_id) &
                        (performance_history[date_column] < current_date) &
                        (performance_history['track_condition'].isin(['soft', 'heavy', 'yielding']))
                    ]
                    
                    if len(bad_condition) > 0:
                        stats['bad_condition_starts'] = len(bad_condition)
                        stats['bad_condition_win_rate'] = (bad_condition['finish_position'] == 1).sum() / len(bad_condition)
            ...
            raise FeatureExtractionError(f"馬場状態別成績特徴量抽出中にエラーが発生しました: {str(e)}")
    ...
    def extract_all_performance_features(
        ...
        try:
            ...
            raise FeatureExtractionError(f"全過去成績特徴量抽出中にエラーが発生しました: {str(e)}")