...
class BaseFeatureExtractor:
    ...
    def extract_horse_basic_features(
        ...
        try:
            ...
            if 'weight_carried' in df.columns:
                ...
                self.numerical_features.extend(['weight_carried', 'weight_carried_diff'])
            ...
            if 'horse_weight' in df.columns:
                ...
                if 'horse_weight_diff' in df.columns:
                    df_features['horse_weight_diff'] = df['horse_weight_diff'].astype(float)
                    # 体重変化率
                    ...
                    self.numerical_features.extend(['horse_weight_diff', 'horse_weight_change_rate'])
            ...
            if 'field_size' in df.columns and 'post_position' in df.columns:
                df_features['post_position_ratio'] = df['post_position'] / df['field_size']
                df_features['is_widest_post'] = (df['post_position'] == df['field_size']).astype(int)
            ...
            if 'days_since_last_race' in df.columns:
                df_features['days_since_last_race'] = df['days_since_last_race'].astype(float)
                # 間隔カテゴリ
            ...
            logger.info(f"馬の基本特徴量抽出完了: 数値特徴量={len(self.numerical_features)}, カテゴリ特徴量={len(self.categorical_features)}")
            
            return df_features
            
        except Exception as e:
            raise FeatureExtractionError(f"馬の基本特徴量抽出中にエラーが発生しました: {str(e)}")
        ...
        try:
            ...
            if 'jockey_id' in df.columns:
                ...
                if jockey_stats is not None and 'jockey_id' in jockey_stats.columns:
                    # 騎手統計をマージ
                    jockey_cols = ['jockey_id', '着順_win_rate', '着順_place_rate', '着順_show_rate']
            ...
            if 'trainer_id' in df.columns:
                ...
                if trainer_stats is not None and 'trainer_id' in trainer_stats.columns:
                    # 調教師統計をマージ
                    trainer_cols = ['trainer_id', '着順_win_rate', '着順_place_rate', '着順_show_rate']
                    trainer_cols = [c for c in trainer_cols if c in trainer_stats.columns]
            ...
            if 'jockey_id' in df.columns and 'trainer_id' in df.columns:
                ...
                # コンビネーションの頻度（同一レース内）
                combo_counts = df_features.groupby(['race_id', 'jockey_trainer_combo']).size()
                df_features['combo_frequency'] = df_features.apply(
                    lambda x: combo_counts.get((x['race_id'], x['jockey_trainer_combo']), 0)
                    if 'race_id' in x else 0,
                    axis=1
                )
            ...
            raise FeatureExtractionError(f"騎手・調教師の特徴量抽出中にエラーが発生しました: {str(e)}")
    
    def extract_race_condition_features(
        self,
        df: pd.DataFrame
    ) -> pd.DataFrame:
        """レース条件の特徴量抽出
        ...
        try:
            ...
            if 'track_type' in df.columns:
                df_features['track_type_encoded'] = df['track_type'].map({'turf': 0, 'dirt': 1})
            ...
            if 'track_condition' in df.columns:
                condition_map = {'firm': 1, 'good': 2, 'yielding': 3, 'soft': 4, 'heavy': 5}
                df_features['track_condition_encoded'] = df['track_condition'].map(condition_map)
                condition_dummies = pd.get_dummies(df['track_condition'], prefix='condition')
            ...
            if 'race_class' in df.columns:
                ...
                df_features['race_class_rank'] = df['race_class'].map(class_rank).fillna(0)
            ...
            if 'race_time' in df.columns:
                ...
                df_features['is_afternoon'] = (df_features['race_hour'] >= 12).astype(int)
            ...
            if 'racecourse' in df.columns:
                ...
                df_features['is_east_course'] = df['racecourse'].isin(east_courses).astype(int)
            ...
            raise FeatureExtractionError(f"レース条件の特徴量抽出中にエラーが発生しました: {str(e)}")
    ...
    def extract_all_base_features(
        ...
        try:
            ...
            raise FeatureExtractionError(f"全基本特徴量抽出中にエラーが発生しました: {str(e)}")
