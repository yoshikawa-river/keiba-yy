...
class RelativeFeatureExtractor:
    ...
    def extract_relative_performance_features(
        self,
        df: pd.DataFrame
    ) -> pd.DataFrame:
        """他馬との相対的な能力差特徴量
        ...
        try:
                    ...
                    for metric, name in metrics.items():
                        if metric in race_group.columns:
                            ...
                            if pd.notna(own_value):
                                # 自分以外の馬の値
                                others = race_group[race_group.index != idx][metric].dropna()
                                
                                if len(others) > 0:
                                    # 相対的な指標
                                    features[f'{metric}_vs_avg'] = own_value - others.mean()
                                    features[f'{metric}_vs_best'] = own_value - others.min() if 'finish' in metric else own_value - others.max()
                                    features[f'{metric}_rank'] = (race_group[metric] >= own_value).sum() if 'finish' not in metric else (race_group[metric] <= own_value).sum()
                                    features[f'{metric}_percentile'] = features[f'{metric}_rank'] / len(race_group)
                                    
                                    # 標準偏差で正規化した差
                                    if others.std() > 0:
                                        features[f'{metric}_zscore'] = (own_value - others.mean()) / others.std()
                                    else:
                                        features[f'{metric}_zscore'] = 0
                    
                    # 総合的な相対強度
                    if 'career_win_rate' in race_group.columns and 'recent5_avg_finish' in race_group.columns:
                        # 複数の指標を組み合わせた総合スコア
                        win_rate_rank = features.get('career_win_rate_rank', len(race_group) / 2)
                        finish_rank = features.get('recent5_avg_finish_rank', len(race_group) / 2)
            ...
            logger.info(f"相対能力差特徴量抽出完了")
            
            return df_features
            
        except Exception as e:
            raise FeatureExtractionError(f"相対能力差特徴量抽出中にエラーが発生しました: {str(e)}")
    
    def extract_odds_based_features(
        self,
        df: pd.DataFrame
    ) -> pd.DataFrame:
        """オッズベースの特徴量抽出
        ...
        try:
            ...
            if 'race_id' in df.columns:
                df_features['popularity_rank'] = df.groupby('race_id')['odds'].rank(method='min')
                df_features['is_favorite'] = (df_features['popularity_rank'] == 1).astype(int)
                df_features['is_top3_favorite'] = (df_features['popularity_rank'] <= 3).astype(int)
                df_features['is_top5_favorite'] = (df_features['popularity_rank'] <= 5).astype(int)
                ...
                # レースごとのオッズ統計
                odds_stats = df.groupby('race_id')['odds'].agg(['mean', 'std', 'min', 'max'])
                ...
                df_features['odds_deviation'] = (df_features['odds'] - df_features['mean']) / df_features['std']
                ...
                if 'career_win_rate' in df.columns:
                    ...
                    # 過小評価されている馬
                    df_features['is_undervalued'] = (df_features['odds_value_gap'] > 0.05).astype(int)
                    # 過大評価されている馬
                    df_features['is_overvalued'] = (df_features['odds_value_gap'] < -0.05).astype(int)
            
            # 複勝オッズ関連（もしあれば）
            if 'place_odds_min' in df.columns and 'place_odds_max' in df.columns:
                df_features['place_odds_range'] = df['place_odds_max'] - df['place_odds_min']
                df_features['place_odds_mid'] = (df['place_odds_min'] + df['place_odds_max']) / 2
                df_features['implied_place_probability'] = 1 / (df_features['place_odds_mid'] + 1)
            
            self.feature_names.extend([
                'odds_log', 'implied_win_probability', 'popularity_rank',
                'is_favorite', 'is_top3_favorite', 'is_top5_favorite',
                'relative_odds', 'odds_deviation'
            ])
            
            if 'career_win_rate' in df.columns:
                self.feature_names.extend(['odds_value_gap', 'is_undervalued', 'is_overvalued'])
            ...
            raise FeatureExtractionError(f"オッズベース特徴量抽出中にエラーが発生しました: {str(e)}")
    ...
    def extract_jockey_trainer_relative_features(
        ...
        try:
                ...
                if 'jockey_win_rate' in race_group.columns:
                    ...
                    for idx in race_group.index:
                        ...
                        if pd.notna(race_group.loc[idx, 'jockey_win_rate']):
                            ...
                            # このレースで最も勝率の高い騎手か
                            features['has_top_jockey'] = (own_rate == jockey_rates.max()).astype(int)
                            # 騎手勝率ランク
                            features['jockey_rank_in_race'] = (jockey_rates >= own_rate).sum()
                            # 相対的な騎手の強さ
                            features['jockey_relative_strength'] = own_rate / jockey_rates.mean() if jockey_rates.mean() > 0 else 1
                        
                        # 調教師の相対評価
                        if 'trainer_win_rate' in race_group.columns and pd.notna(race_group.loc[idx, 'trainer_win_rate']):
                            trainer_rates = race_group['trainer_win_rate'].dropna()
                            own_rate = race_group.loc[idx, 'trainer_win_rate']
                            
                            features['has_top_trainer'] = (own_rate == trainer_rates.max()).astype(int)
                            features['trainer_rank_in_race'] = (trainer_rates >= own_rate).sum()
                            features['trainer_relative_strength'] = own_rate / trainer_rates.mean() if trainer_rates.mean() > 0 else 1
            ...
            raise FeatureExtractionError(f"騎手・調教師相対特徴量抽出中にエラーが発生しました: {str(e)}")
    
    def extract_position_relative_features(
        self,
        df: pd.DataFrame
    ) -> pd.DataFrame:
        """枠順・馬番の相対的な特徴量
        ...
        try:
            ...
            for race_id, race_group in df.groupby('race_id'):
                ...
                for idx in race_group.index:
                    ...
                    # 相対的な枠順位置（0-1で正規化）
                    features['normalized_position'] = (position - 1) / (field_size - 1) if field_size > 1 else 0.5
                    ...
                    # 内枠率・外枠率
                    features['inner_position_ratio'] = (field_size - position + 1) / field_size
                    ...
                    if 'career_win_rate' in race_group.columns:
                        ...
                        if pd.notna(own_win_rate):
                            ...
                            if pos_idx > 0:
                                ...
                                inner_win_rate = race_group.loc[inner_idx, 'career_win_rate']
                                if pd.notna(inner_win_rate):
                                    features['win_rate_diff_inner'] = own_win_rate - inner_win_rate
                            
                            # 外側の馬との差
                            if pos_idx < field_size - 1:
                                outer_idx = sorted_group.index[pos_idx + 1]
                                outer_win_rate = race_group.loc[outer_idx, 'career_win_rate']
                                if pd.notna(outer_win_rate):
                                    features['win_rate_diff_outer'] = own_win_rate - outer_win_rate
            ...
            raise FeatureExtractionError(f"枠順相対特徴量抽出中にエラーが発生しました: {str(e)}")
    
    def extract_pace_relative_features(
        self,
        df: pd.DataFrame
    ) -> pd.DataFrame:
        """ペース・脚質の相対的な特徴量
        ...
        try:
            ...
            for race_id, race_group in df.groupby('race_id'):
                ...
                for idx in race_group.index:
                    ...
                    # 同じ脚質の馬の数
                    features['same_style_count'] = style_counts.get(own_style, 0) - 1  # 自分を除く
                    features['same_style_ratio'] = features['same_style_count'] / (total_horses - 1)
                    
                    # 脚質の競合度
                    if own_style == '逃げ':
                        features['style_competition'] = features['front_runner_ratio']
                        features['is_sole_front_runner'] = (features['same_style_count'] == 0).astype(int)
                    elif own_style == '先行':
                        features['style_competition'] = features['front_runner_ratio'] + features['stalker_ratio']
                    elif own_style == '差し':
            ...
            raise FeatureExtractionError(f"ペース相対特徴量抽出中にエラーが発生しました: {str(e)}")
    ...
    def extract_all_relative_features(
        ...
        try:
            ...
            raise FeatureExtractionError(f"全相対特徴量抽出中にエラーが発生しました: {str(e)}")