...
class RaceFeatureExtractor:
    ...
    def extract_race_level_features(
        ...
        try:
            ...
            if 'race_class' in df.columns:
                ...
                self.feature_names.extend(['race_class_rank', 'is_graded_race', 'is_g1_race'])
            ...
            if historical_races is not None and 'race_name' in df.columns:
                ...
                for idx, row in df.iterrows():
                    ...
                    same_race = historical_races[
                        ...
                        (historical_races['distance'] == distance) if distance else True
                    ]
            ...
            if 'age_restriction' in df.columns:
                age_dummies = pd.get_dummies(df['age_restriction'], prefix='age_restrict')
            ...
            if 'sex_restriction' in df.columns:
                df_features['has_sex_restriction'] = (~df['sex_restriction'].isna()).astype(int)
                df_features['is_fillies_only'] = (df['sex_restriction'] == '牝').astype(int)
                self.feature_names.extend(['has_sex_restriction', 'is_fillies_only'])
            
            logger.info(f"レースレベル特徴量抽出完了")
            
            return df_features
            
        except Exception as e:
            raise FeatureExtractionError(f"レースレベル特徴量抽出中にエラーが発生しました: {str(e)}")
    
    def extract_field_competition_features(
        self,
        df: pd.DataFrame
    ) -> pd.DataFrame:
        """出走馬の競争レベル特徴量抽出
        ...
        try:
                    ...
                    for idx in race_group.index:
                        ...
                        if 'career_win_rate' in race_group.columns:
                            ...
                            horse_stats['relative_strength_rank'] = (
                                race_group['career_win_rate'].rank(ascending=False).loc[idx]
                            )
                            # 勝率の偏差値
            ...
            if 'odds' in df.columns:
                ...
                if 'race_id' in df.columns:
                    # レースごとの統計
                    race_odds_stats = df.groupby('race_id')['odds'].agg(['mean', 'std', 'min'])
                    ...
                    df_features['is_favorite'] = (df_features['odds'] == df_features['min']).astype(int)
            ...
            raise FeatureExtractionError(f"出走馬競争レベル特徴量抽出中にエラーが発生しました: {str(e)}")
    ...
    def extract_pace_features(
        ...
        try:
            ...
            if 'distance' in df.columns:
                ...
                pace_dummies = pd.get_dummies(df_features['expected_pace'], prefix='pace')
            ...
            raise FeatureExtractionError(f"ペース予想特徴量抽出中にエラーが発生しました: {str(e)}")
    ...
    def extract_position_advantage_features(
        ...
        try:
            ...
            if 'racecourse' in df.columns and 'post_position' in df.columns:
                ...
                for idx, row in df.iterrows():
                    ...
                    if course in course_advantages:
                        ...
                        # 相対的な枠順位置（0-1）
                        relative_position = (position - 1) / (field_size - 1) if field_size > 1 else 0.5
                        ...
                        # 内枠・外枠の絶対的な判定
                        features['is_inner_draw'] = 1 if position <= field_size * 0.3 else 0
                        features['is_outer_draw'] = 1 if position >= field_size * 0.7 else 0
                        features['is_middle_draw'] = 1 - features['is_inner_draw'] - features['is_outer_draw']
                    else:
            ...
            raise FeatureExtractionError(f"枠順有利不利特徴量抽出中にエラーが発生しました: {str(e)}")
    
    def extract_seasonal_race_features(
        self,
        df: pd.DataFrame
    ) -> pd.DataFrame:
        """季節・時期による特徴量抽出
        ...
        try:
            ...
            if 'race_date' in df.columns:
                ...
                # 季節ダミー変数
                season_dummies = pd.get_dummies(df_features['race_season'], prefix='season')
                ...
                df_features['is_year_end'] = (
                    df_features['race_month'] == 12
                ).astype(int)
                
                self.feature_names.extend([
                    'race_month', 'is_spring_classics', 'is_autumn_classics',
                    'is_summer_series', 'is_year_end'
                ])
            ...
            raise FeatureExtractionError(f"季節・時期特徴量抽出中にエラーが発生しました: {str(e)}")
    ...
    def extract_all_race_features(
        ...
        try:
            ...
            # 枠順有利不利特徴量
            df_features = self.extract_position_advantage_features(df_features, course_statistics)
            ...
            raise FeatureExtractionError(f"全レース特徴量抽出中にエラーが発生しました: {str(e)}")