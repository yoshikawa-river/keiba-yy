...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("logs/mykeibadb_sync.log"), logging.StreamHandler()],
)
...
class MykeibaDBSync:
    ...
    def connect_source(self):
        ...
        try:
            ...
            logging.exception(f"Failed to connect to mykeibaDB: {e}")
            raise
    ...
    def connect_target(self):
        ...
        try:
            ...
            logging.exception(f"Failed to connect to local MySQL: {e}")
            raise
    ...
    def get_table_structure(self, conn, table: str) -> str:
        ...
        try:
            ...
            logging.exception(f"Failed to get structure for {table}: {e}")
            return None
        finally:
            cursor.close()
    ...
    def sync_table(self, table: str, where_clause: str | None = None):
        """個別テーブルを同期"""
        ...
        try:
            ...
            logging.exception(f"Error syncing {table}: {e}")
            target_conn.rollback()
            raise
        except Exception as e:
            logging.exception(f"Unexpected error syncing {table}: {e}")
            target_conn.rollback()
            raise
        finally:
            source_conn.close()
        ...
        for table in self.sync_tables:
            try:
                ...
                if match:
                    ...
                    logging.warning(
                        f"{status} {table}: Source={source_count}, Target={target_count} (MISMATCH)"
                    )

            except mysql.connector.Error as e:
                logging.exception(f"Error verifying {table}: {e}")
