...
class TestBatchProcessor:
    ...
    def test_validate_data_quality_large_file(self, processor, mock_import_dir):
        ...
        assert any("ファイルサイズが大きすぎます" in issue["issue"] 
                  for issue in quality_report["issues"])
        assert any("行数が多すぎます" in issue["issue"] 
                  for issue in quality_report["issues"])
    ...
    def test_validate_data_quality_empty_file(self, processor, mock_import_dir):
        ...
        # 空ファイルの問題が検出される
        assert len(quality_report["issues"]) > 0
        assert any("ファイルが空の可能性" in issue["issue"] 
                  for issue in quality_report["issues"])
        assert any("ヘッダーが検出されませんでした" in issue["issue"] 
                  for issue in quality_report["issues"])
    ...
    def test_process_all(self, mock_process_file, processor, mock_csv_files):
        """全ファイル処理のテスト"""
        # ファイル検出のモック
        with patch.object(processor.file_detector, 'detect_files', return_value=mock_csv_files):
            # ファイル処理のモック
